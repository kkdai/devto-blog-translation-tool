...
func (c *Client) TranslateTitle(ctx context.Context, title string) (string, error) {
	...
	model.SetTopP(0.95)
	model.SetTopK(40)
	// gemini-3-flash-preview is a thinking model: internal reasoning tokens count
	// against MaxOutputTokens. A tight limit (e.g. 256) gets consumed by thinking
	// and truncates the title mid-output, so allow generous headroom. Billing is
	// on tokens actually used, so a high ceiling has no cost downside.
	model.SetMaxOutputTokens(4096)

	prompt := fmt.Sprintf(`Translate this article title to English.
If it's already in English, return it as-is.
Keep it concise and natural.
CRITICAL: The translated title MUST be under 128 characters long.
